...
(function () {
    // Store references to our elements so we don't lose them during navigation
    let floatingPanel = null;
    let toggleButton = null;

    // Main initialization function - we'll call this on document ready AND after Inertia navigations
    function initializeStatsHelper() {
        console.log("Initializing DartConnect Stats Helper...");

        // Create toggle button if it doesn't exist
        if (!toggleButton) {
            toggleButton = $(`
                <button id="toggle-display" 
                    class="fixed bottom-button right-5 !bg-pink-500 !text-white px-4 py-2 rounded-full shadow-lg 
                    hover:bg-pink-600 transition-all z-40">
                    Show Stats 🎯
                </button>`);
        ...
        if (!floatingPanel) {
            ...
            <div id="floating-panel"
                 class="sticky top-0 bg-pink-50 shadow-xl border border-pink-300 p-4 rounded-l-2xl w-96
                ...
                <div id="stats-header" class="grid grid-cols-12 mt-2 text-xs pb-2 border-b border-pink-300">
                    ...
                    <div class="text-pink-600 col-span-4">Highest Avg:</div>
                    <div id="highest-player" class="col-span-8"></div>
                    <div class="text-pink-600 col-span-4">Lowest Avg:</div>
                    <div id="lowest-player" class="col-span-8"></div>
                    <div class="text-pink-600 col-span-4">Tournament Avg:</div>
                    <div id="tournament-avg" class="col-span-8"></div>
                    <div class="text-pink-600 col-span-4">Players count:</div>
                    <div id="player-count" class="col-span-8"></div>
                    <div class="text-pink-600 col-span-4">Games count:</div>
                    <div id="games-count" class="col-span-8"></div>
                </div>
                ...
                <input id="filter-player-averages" class="
                        ...
                        hover:bg-pink-50" type="text" placeholder="✨ Filter Players ✨"/>
        ...
        // Check if we're on an event page and position our elements appropriately
        positionElementsForCurrentPage();

        // Re-attach event handlers
        attachEventHandlers();
    }

    // Position elements based on current URL
    function positionElementsForCurrentPage() {
        // Add to body first to ensure they exist
        $('body').append(floatingPanel);
        $('body').append(toggleButton);
        ...
        if (location.href.includes("/event/")) {
            ...
            let header = $('#header');
            let tournament_wrapper = $(header).next();

            // Ensure the tournament wrapper has the right classes
            tournament_wrapper.removeClass('flex-grow').addClass('flex justify-center');

            // Move the floating panel to the tournament wrapper
            tournament_wrapper.append(floatingPanel);
    ...
    function updateStats() {
        ...
        <div class="flag_count grid grid-cols-2 items-center gap-2 border !border-pink-300 rounded-md cursor-pointer"
        data-flag="${flag}">
    ...
    function attachEventHandlers() {
        ...
        $(document).off('click', "#toggle-display, #refresh-stats").on('click', "#toggle-display, #refresh-stats", function() {
            let header = $('#header');
            $(`button[aria-label="Open the menu of DartConnect sites and apps"]`).hide();
            let tournament_wrapper = $(header).next().next();
            console.log("tournament_wrapper");
            console.log(tournament_wrapper);
            tournament_wrapper.removeClass('flex-grow').addClass('flex justify-between pl-4');
            tournament_wrapper.append(floatingPanel);

            // Click the "Show +" button if it exists
            $("span").filter(function() {
                return $(this).text().trim() === "Show +";
            }).closest("button").click();

            // Wait 500ms before updating stats
            setTimeout(function() {
                floatingPanel.removeClass("hidden").addClass("translate-y-0 opacity-100");
                updateStats();
            }, 500);
        });

        // Close button with instant hiding after animation
        $(document).off('click', "#close-panel").on('click', "#close-panel", function() {
            floatingPanel.addClass("translate-y-72 opacity-0").delay(500).queue(function(next) {
                $(this).addClass("hidden").removeClass("translate-y-72 opacity-0");
                next();
            });

            $(`button[aria-label="Open the menu of DartConnect sites and apps"]`).show();
        });

        // Filter out players by flag
        $(document).off('click', '.flag_count').on('click', '.flag_count', function() {
        ...
        // Filter out players
        $(document).off('click', '.player_name_select').on('click', '.player_name_select', function() {
            // Get the player name
        ...
        $(document).off('input', '#filter-player-averages').on('input', '#filter-player-averages', function() {
            let query = $(this).val().toLowerCase();
            $('.player_name_select').each(function() {
                let playerRow = $(this);
                let playerName = playerRow.attr('data-player-name')?.toLowerCase() || "";
    ...
    $(document).ready(function() {
        console.log("Document ready, initializing stats helper...");
        addStyles();
        initializeStatsHelper();
    });

    // Initialize when Inertia navigation completes
    document.addEventListener("inertia:start", () => {
        console.log("🔄 Inertia navigation started...");
    });

    document.addEventListener("inertia:finish", (event) => {
        console.log("🔄 Inertia navigation finished, re-initializing stats helper...");
        // Small delay to ensure DOM is updated
        setTimeout(() => {
            initializeStatsHelper();

            // If we're on an event page and stats panel was previously visible, update the stats
            if (location.href.includes("/event/") && floatingPanel && !floatingPanel.hasClass("hidden")) {
                updateStats();
            }
        }, 300);
    });
